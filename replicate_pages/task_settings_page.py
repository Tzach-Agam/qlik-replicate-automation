...
class TaskSettings:
    ...
    def target_schema(self, target_schema: str = None):
        """ Enter the 'target schema' for the replication task. The 'target schema' is the schema in which the replicated
            ...
            :param target_schema: The name of the 'target schema'"""
        ...
        if target_schema_value:
            ...
            if target_schema:
                target_schema_element.send_keys(target_schema)
            else:
                target_schema_element.send_keys(self.config.get_default_schemas()[1])
        else:
            if target_schema:
                target_schema_element.send_keys(target_schema)
            else:
    ...
    def control_schema(self, control_schema: str = None):
        """ Enter the 'control tables schema' for the replication task. The 'control tables schema' is a schema that is
            ...
            :param control_schema: The name of the 'control tables schema'"""
        ...
        if control_schema_value:
            ...
            if control_schema:
                control_schema_element.send_keys(control_schema)
            else:
    ...
    def set_task_settings_general(self, target_schema: str = None, control_schema: str = None):
