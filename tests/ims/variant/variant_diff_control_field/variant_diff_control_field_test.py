...
def test_variant_diff_control_field(ims_test):
    ...
    ims_test.ims_db.cursor.execute(
        f"UPDATE  \"DEVPCB\".\"STRUCT2\" SET VARIANT_SELECTOR = 'BE' WHERE SKEY = '00000000000000000001'")
    ims_test.ims_db.cursor.execute(
        f"UPDATE  \"DEVPCB\".\"STRUCT2\" SET VARIANT_SELECTOR = 'BE', FIELD_B1 = 'UPDA' WHERE SKEY = '00000000000000000002'")
    ims_test.ims_db.cursor.execute(
        f"UPDATE  \"DEVPCB\".\"STRUCT2\" SET VARIANT_SELECTOR = 'BE', FIELD_B1 = 'UPDA', FIELD_B2 = 12345 WHERE SKEY = '00000000000000000003'")
    ims_test.ims_db.cursor.execute(
    ...
    ims_test.target_db.export_schema_data_to_csv(ims_test.target_schema,
                                                 ims_test.good_files_dir + f"\\IMS_2_{ims_test.target_db.config['endpoint']}_VARIANT_DIFF_CON_FIELD.csv")
    compare_files(ims_test.good_files_dir + f"\\IMS_2_{ims_test.target_db.config['endpoint']}_VARIANT_DIFF_CON_FIELD.good",
                  ims_test.good_files_dir + f"\\IMS_2_{ims_test.target_db.config['endpoint']}_VARIANT_DIFF_CON_FIELD.csv")