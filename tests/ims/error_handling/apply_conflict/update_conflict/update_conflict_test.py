...
def test_update_conflict(ims_test):
    """Test for UPDATE conflict and Error Handling"""
    ...
    ims_test.ims_db.cursor.execute(
        f"UPDATE \"DEVPCB\".\"STRUCT2\" SET BASIC_TABLE_1_COL1_CHAR = 'UPDATE_CONFLICT', BASIC_TABLE_1_COL2_NUM = 9999 WHERE SKEY = 'KEY2'")
    ims_test.ims_db.cursor.execute(
        f"UPDATE \"DEVPCB\".\"STRUCT2\" SET BASIC_TABLE_1_COL1_CHAR = 'UPDATE_CONFLICT', BASIC_TABLE_1_COL2_NUM = 9999 WHERE SKEY = 'KEY3'")
    ims_test.ims_db.connection.commit()
    ...
    ims_test.target_db.export_schema_data_to_csv(ims_test.target_schema,
                                                 ims_test.good_files_dir + f"\\IMS_2_{ims_test.target_db.config['endpoint']}_AC_UPDATE.csv")
    ...
    compare_files(ims_test.good_files_dir + f"\\IMS_2_{ims_test.target_db.config['endpoint']}_AC_UPDATE.good",
                  ims_test.good_files_dir + f"\\IMS_2_{ims_test.target_db.config['endpoint']}_AC_UPDATE.csv")
