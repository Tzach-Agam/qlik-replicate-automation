...
def test_data_error_trg_fl(ims_test):
    ...
    ims_test.monitor_page.wait_for_message_in_ui("Table 'public'.'STRUCT2' (subtask 1 thread 1) is suspended")
    ...
    log_finder(ims_test.task_logs_dir + f"\\reptask_{ims_test.task_name}.log",
               "]W:  Table 'public'.'STRUCT2' (subtask 1 thread 1) is suspended",
               ims_test.good_files_dir + f"\\IMS_2_{ims_test.target_db.config['endpoint']}DATA_ERROR_FL.csv")
    log_finder(ims_test.task_logs_dir + f"\\reptask_{ims_test.task_name}.log",
               "]E:  Handling End of table 'replicate_selenium_target'.'BASIC_TABLE' loading failed",
               ims_test.good_files_dir + f"\\IMS_2_{ims_test.target_db.config['endpoint']}DATA_ERROR_FL.csv")
