...
def test_resume_task(ims_test):
    ...
    ims_test.ims_db.cursor.execute(
        f"UPDATE \"DVPCB\".\"STRUCT2\"  SET BASIC_TABLE_1_COL1_CHAR = 'UPDATE' WHERE SKEY = 'KEY9'")
    ims_test.ims_db.cursor.execute(f"DELETE FROM \"DVPCB\".\"STRUCT2\"  WHERE SKEY = 'KEY10'")
    ims_test.ims_db.connection.commit()
    ...
    finalize_test(ims_test, f"IMS_2_{ims_test.target_db.config['endpoint']}_RESUME_TASK")