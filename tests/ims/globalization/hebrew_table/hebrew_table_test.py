...
def test_hebrew_table(ims_test):
    ...
    ims_test.ims_db.cursor.execute(
        f"INSERT INTO \"DEVPCB\".\"STRUCT2\"  (ROOT_ROOTID, SKEY, HEBREW_TABLE_1_COL1, HEBREW_TABLE_1_COL2)"
    ...
    ims_test.ims_db.cursor.execute(
        f"UPDATE \"DEVPCB\".\"STRUCT2\" SET HEBREW_TABLE_1_COL1 = X'D7A7D7A7D7A7D7A7D7A7' WHERE SKEY = 'KEY1'")
    ims_test.ims_db.cursor.execute(
        f"UPDATE \"DEVPCB\".\"STRUCT2\" SET HEBREW_TABLE_1_COL1 = X'D7A7D7A7D7A7D7A7D7A7' WHERE SKEY = 'KEY2'")
    ...
    ims_test.target_db.export_schema_data_to_csv(ims_test.target_schema,
                                                 ims_test.good_files_dir + f"\\IMS_2_{ims_test.target_db.config['endpoint']}_HEBREW_TABLE.csv")
    compare_files(ims_test.good_files_dir + f"\\IMS_2_{ims_test.target_db.config['endpoint']}_HEBREW_TABLE.good",
                  ims_test.good_files_dir + f"\\IMS_2_{ims_test.target_db.config['endpoint']}_HEBREW_TABLE.csv")