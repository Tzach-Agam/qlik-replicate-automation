...
def test_control_tables(ims_test):
    """Replication of control tables with IMS"""
    ...
    ims_test.ims_db.cursor.execute(
        f"UPDATE \"DVPCB\".\"STRUCT2\"  SET BASIC_TABLE_1_COL1_CHAR = 'UPDATE' WHERE SKEY = 'KEY1'")
    ims_test.ims_db.cursor.execute(
        f"UPDATE \"DVPCB\".\"STRUCT2\"  SET BASIC_TABLE_1_COL1_CHAR = 'UPDATE' WHERE SKEY = 'KEY2'")
    ims_test.ims_db.cursor.execute(
        f"UPDATE \"DVPCB\".\"STRUCT2\"  SET BASIC_TABLE_1_COL1_CHAR = 'UPDATE' WHERE SKEY = 'KEY6'")
    ims_test.ims_db.cursor.execute(
        f"UPDATE \"DVPCB\".\"STRUCT2\"  SET BASIC_TABLE_1_COL1_CHAR = 'UPDATE' WHERE SKEY = 'KEY7'")
    ims_test.ims_db.cursor.execute(f"DELETE FROM \"DVPCB\".\"STRUCT2\"  WHERE SKEY = 'KEY3'")
    ims_test.ims_db.cursor.execute(f"DELETE FROM \"DVPCB\".\"STRUCT2\"  WHERE SKEY = 'KEY8'")
    ims_test.ims_db.connection.commit()
    ...
    ims_test.target_db.export_schema_data_to_csv(ims_test.target_schema,
                                                 ims_test.good_files_dir + f"\\IMS_2_{ims_test.target_db.config['endpoint']}_CONTROL_TABLES.csv")
    ims_test.target_db.append_schema_data_to_csv(ims_test.control_schema,
                                                 ims_test.good_files_dir + f"\\IMS_2_{ims_test.target_db.config['endpoint']}_CONTROL_TABLES.csv")

    # compare_files(ims_test.good_files_dir + f"\\IMS_2_{ims_test.target_db.config['endpoint']}_CONTROL_TABLES.good",
    #               ims_test.good_files_dir + f"\\IMS_2_{ims_test.target_db.config['endpoint']}_CONTROL_TABLES.csv")