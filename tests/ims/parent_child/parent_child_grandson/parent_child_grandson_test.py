...
def test_parent_child_grandson(ims_test):
    ...
    ims_test.ims_db.cursor.execute("DELETE FROM \"DEVPCB\".\"PARENT\"")
    ims_test.ims_db.cursor.execute("DELETE FROM \"DEVPCB\".\"CHILD\"")
    ...
    for i in range(1, 5):
        ims_test.ims_db.cursor.execute(
            f"INSERT INTO \"DEVPCB\".\"PARENT\" (ROOT_ROOTID, PKEY, FIRST_NAME, MIDDLE_NAME, LAST_NAME)"
    ...
    for i in range(1, 5):
        ...
        ims_test.ims_db.connection.commit()
    ims_test.ims_db.cursor.execute(
        f"INSERT INTO \"DEVPCB\".\"CHILD\" (ROOT_ROOTID, PARENT_PKEY, CKEY, COUNTRY, CITY, STREET_NAME, BUILDING_NO, ZIP_CODE) VALUES"
    ...
    ims_test.ims_db.cursor.execute(f"DELETE FROM \"DEVPCB\".\"ROOT\" WHERE ROOTID = 'ROOT000PC1'")
    ims_test.ims_db.cursor.execute(f"DELETE FROM \"DEVPCB\".\"PARENT\" WHERE ROOT_ROOTID = 'ROOT000PC2'")
    ims_test.ims_db.cursor.execute(f"DELETE FROM \"DEVPCB\".\"PARENT\" WHERE PKEY = 'PKEY3'")
    ims_test.ims_db.cursor.execute(f"DELETE FROM \"DEVPCB\".\"CHILD\" WHERE PARENT_PKEY = 'PKEY4'")
    ims_test.ims_db.connection.commit()
    ...
    ims_test.target_db.export_schema_data_to_csv(ims_test.target_schema,
                                                 ims_test.good_files_dir + f"\\IMS_2_{ims_test.target_db.config['endpoint']}Parent_Child_Grandson.csv")
    compare_files(ims_test.good_files_dir + f"\\IMS_2_{ims_test.target_db.config['endpoint']}Parent_Child_Grandson.good",
                  ims_test.good_files_dir + f"\\IMS_2_{ims_test.target_db.config['endpoint']}Parent_Child_Grandson.csv")