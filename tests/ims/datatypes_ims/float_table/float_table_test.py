...
def test_float_datatype(ims_test):
    ...
    ims_test.ims_db.cursor.execute(
        f"INSERT INTO \"DEVPCB\".\"ALLTYPES\" (ROOT_ROOTID, ID) VALUES ('ROOT000000', 18)")
    ims_test.ims_db.cursor.execute(
        f"UPDATE  \"DEVPCB\".\"ALLTYPES\" SET FLOAT_TABLE_1_DOUBLE_COL = 1.082699 WHERE ID = 12")
    ims_test.ims_db.cursor.execute(
        f"UPDATE  \"DEVPCB\".\"ALLTYPES\" SET FLOAT_TABLE_1_FLOAT_COL = 1.3456 WHERE ID = 11")
    ims_test.ims_db.cursor.execute(
        f"UPDATE  \"DEVPCB\".\"ALLTYPES\" SET FLOAT_TABLE_1_DOUBLE_COL = 3.1415926535 WHERE ID = 10")
    ims_test.ims_db.cursor.execute(
        f"DELETE FROM \"DEVPCB\".\"ALLTYPES\" WHERE ID = 9")
    ims_test.ims_db.connection.commit()
