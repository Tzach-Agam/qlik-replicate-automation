...
def test_time_datatype(ims_test):
    ...
    ims_test.ims_db.cursor.execute(
        f"INSERT INTO \"DVPCB\".\"ALLTYPES\" (ROOT_ROOTID, ID, TIME_TABLE_1_TIMESTAMP_COL, TIME_TABLE_1_TIME_COL)"
    ...
    ims_test.ims_db.cursor.execute(
        f"UPDATE  \"DVPCB\".\"ALLTYPES\" SET TIME_TABLE_1_TIMESTAMP_COL = '1995-05-07 06:07:08' WHERE ID = 6")
    ims_test.ims_db.cursor.execute(
        f"UPDATE  \"DVPCB\".\"ALLTYPES\" SET TIME_TABLE_1_TIME_COL = '06:07:08' WHERE ID = 5")
    ims_test.ims_db.cursor.execute(
    ...
    finalize_test(ims_test, f"IMS_2_{ims_test.target_db.config['endpoint']}_Time")